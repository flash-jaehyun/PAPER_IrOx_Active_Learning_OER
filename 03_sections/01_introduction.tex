...
\cite{Kirklin2015}
but these datasets are systematically biased towards known materials or hypothetical materials derived from common crystal protoypes.
%
...
In the past, the most common approaches to crystal structure prediction relied on global optimization routines,
in which local and global minima are found via numerical optimization algorithms that operate within the highly dimensional potential energy surface.
%
Classical methods such as simulated annealing, etc. etc. and more recent advances utilizing evolutionary algorithms such as genetic algorithms, particle swarm optimization, and TEMP all fall under this umbrella.
%
% TODO Find solid example of shortcomings of classical GO schemes
These approaches are computationally expensive and become increasingly intractable as the number and types of elements considered increases, due to the number of potential polymorphs increasing exponentially with system size~\cite{Stillinger1999}.
%
The class of structurally diverse metal-oxides are a prime example of a chemical space for which structural prediction is non-trivial,
as these systems tend to configure into a variety of local coordination motiffs
(octahedra, tetrahedra, cubic, etc.)
whose packing arrangments can vary drastically.
% __|
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Active learning (AL) frameworks in conjuction with surrogate models have emerged as an effective approach to successfully speed up various problems in materials science,
and hence have the potential to deliver an efficient alternative approache,
complimentary to existing crystal structure methods.
...
%
The method does not rely on existence of past DFT data,
but instead sequentially generates the minimum amount of data to effectively search within a structurally diverse space of candidate structures generated from crystal structure prototypes.
%
...
In particular, \rIrOtwo (\ce{Ir^{4+}} oxidation state), is the most stable form of iridium-oxide at standard conditions,
...
Previous studies on SrIrO\textsubscript{3} electrocatalyst for the OER demonstrated that Sr leaching might leave behind a highly oxidized \ce{Ir^{6+}} species which was argued to be responsible for the observed OER activity.
\cite{Seitz2016}
%
