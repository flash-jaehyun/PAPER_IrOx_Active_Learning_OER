...
\textbf{A and B do not appear in the figure}
...
whereby structural uniqueness is defined by the element-nonspecific stoicheometry (\ABtwo, \ABthree, etc.), space group symmetry and Wyckoff positions of the atoms within the unit cell. \textbf{this needs another sentence here to explain what this means more plainly}
%
Imposing structural uniqueness \textbf{you never define what this means} reduces the number of entries considerably to \num{697} and \num{259} structures for the \ABtwo and \ABthree stoichiometries, respectively.
The orders of magnitude reduction between all structures and unique structures is a consequence of the lack of structural diversity in the OQMD and MP databases.
%
% NUMBER
\textbf{Write in active voice where possible}
...
this was then followed by isotropic expansion/contraction of the unit cell to accommodate the atomic radii of iridium and oxygen, \textbf{details in SI XYZ}.
%
% NUMBER
...
%
Computational details for the bulk structure relaxations can be found in \textbf{SI XYZ} (SI Bulk polymorph DFT optimization).
%
Of the \num{823} structures, \num{736} were able to be fully optimized, while \num{87}, primarily \ABtwo structures were not able to be converged due to highly voluminous and unphysical initial structures \textbf{Unphysical how?  Do not make sense for atoms like Ir and O?  Needs some more information here or link to an SI section on analysis of these structures.}.
%
...
% | - Paragraph start
%
The candidate data set was featurized using the Voronoi tessellation fingerprinting scheme developed by Ward \latin{et al.} \cite{Ward2017} which produces a \num{271} length feature vector for each material that is invariant to isotropic expansions/contractions of the lattice and to a degree, insensitive to the exact atomic coordinates of the atoms. \textbf{in one sentence you still need to explain what the physical meaning of the features it produces are}
%
% How many columns of the 271 are redundant if stoich and composition are frozen
\textbf{don't use herein so much, it is almost never necessary and is not the way to write a sentence in active voice}
